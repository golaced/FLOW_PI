...
#define patch_size   (2*half_window_size + 1)
...
void getSubPixel_gray(int* Patch, unsigned char* frame_buf, int center_x, int center_y, int half_window_size1, int subpixel_factor)
{
  int x, y, x_0, y_0, x_0_or, y_0_or, i, j, window_size, alpha_x, alpha_y, max_x, max_y;
  //int printed, limit;
  unsigned int ix1, ix2, Y1;
...
//          Y = ((unsigned int)frame_buf[ix2+1] + (unsigned int)frame_buf[ix2+3]) >> 1;
        ...
        Y1 = (unsigned int)frame_buf[ix2+1];
        Patch[ix1] = (int) Y1;
      }
      else
      {
...
//        ix2 = uint_index(x_0_or, y_0_or);
//        Y = ((unsigned int)frame_buf[ix2+1] + (unsigned int)frame_buf[ix2+3]) >> 1;
        Patch[ix1] = (subpixel_factor - alpha_x) * (subpixel_factor - alpha_y) * ((int) Y1);
...
//        Y = ((unsigned int)frame_buf[ix2+1] + (unsigned int)frame_buf[ix2+3]) >> 1;
        ...
        Patch[ix1] += alpha_x * (subpixel_factor - alpha_y) * ((int) Y1);
...
//        Y = ((unsigned int)frame_buf[ix2+1] + (unsigned int)frame_buf[ix2+3]) >> 1;
        ...
        Patch[ix1] += (subpixel_factor - alpha_x) * alpha_y * ((int) Y1);
...
//        Y = ((unsigned int)frame_buf[ix2+1] + (unsigned int)frame_buf[ix2+3]) >> 1;
        ...
        Patch[ix1] += alpha_x * alpha_y * ((int) Y1);
...
void getGradientPatch(int* Patch, int* DX, int* DY, int half_window_size1)
{
  unsigned int ix1, ix2;
  int x, y, padded_patch_size, Y1, Y2;
...
int calculateG(int* G, int* DX, int* DY, int half_window_size1)
{
  //int patch_size;
 ...
 // patch_size = 2 * half_window_size + 1;
  ...
  // allocate memory:
  int DXX[patch_size * patch_size];// = (int *) malloc(patch_size * patch_size * sizeof(int));
  int DXY[patch_size * patch_size];// = (int *) malloc(patch_size * patch_size * sizeof(int));
  int DYY[patch_size * patch_size];// = (int *) malloc(patch_size * patch_size * sizeof(int));
...
int opticFlowLK(unsigned char * new_image_buf, unsigned char * old_image_buf, int* p_x, int* p_y, int n_found_points, int imW, int imH, int* new_x, int* new_y, int* status, int half_window_size1, int max_iterations)
{
	...
	int b_x, b_y, padded_patch_size, error;
	unsigned int ix1, ix2;
...
//	IMG_HEIGHT = imH;
	...
	int I_padded_neighborhood[patch_size * patch_size];// = (int *) malloc(padded_patch_size * padded_patch_size * sizeof(int));
	int I_neighborhood[patch_size * patch_size];// = (int *) malloc(patch_size * patch_size * sizeof(int));
	int J_neighborhood[patch_size * patch_size];// = (int *) malloc(patch_size * patch_size * sizeof(int));
	//if(I_padded_neighborhood == 0 || I_neighborhood == 0 || J_neighborhood == 0)
	//	return NO_MEMORY;

	int DX[patch_size * patch_size];// = (int *) malloc(patch_size * patch_size * sizeof(int));
	int DY[patch_size * patch_size];//= (int *) malloc(patch_size * patch_size * sizeof(int));
	int IDDX[patch_size * patch_size];// = (int *) malloc(patch_size * patch_size * sizeof(int));
	int IDDY[patch_size * patch_size];// = (int *) malloc(patch_size * patch_size * sizeof(int));
	int ImDiff[patch_size * patch_size];// = (int *) malloc(patch_size * patch_size * sizeof(int));
