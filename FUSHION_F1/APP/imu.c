...
{	static u8 init;
	...
	if(mag_norm_xyz==0)
		...
		if( mag_norm_xyz != 0)
	{
		mag_tmp.x += mag_norm_tmp *( (float)hx /( mag_norm_xyz ) - mag_tmp.x);
		mag_tmp.y += mag_norm_tmp *( (float)hy /( mag_norm_xyz ) - mag_tmp.y);	
		mag_tmp.z += mag_norm_tmp *( (float)hz /( mag_norm_xyz ) - mag_tmp.z);	
	}
